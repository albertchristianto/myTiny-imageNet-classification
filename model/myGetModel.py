...
def getModels(trunk='myVGG16', numClass = 0, pretrained_choice=False):
	'''this program is used to build CNN model with input size 56x56x3'''
	if trunk == 'myVGG16' and numClass!= 0:
		myModel = myVGG16(numClass)
		...
		myModel = myAlexnet(numClass)
	elif trunk == 'mine' and numClass!= 0:
		myModel = myOwnModel(numClass)
	elif trunk == 'myResNet18' and numClass!= 0:
		myModel = myResNet18(pretrained=pretrained_choice, num_classes=numClass)
	else:
